...
import { generateActivityName, ActivityNamingContext } from '@/utils/activityNaming';
import { analyzeGPSTrack, inferActivityTypeFromGPS, normalizeSportMode } from '@/utils/activityDetection';
...
export function useActivities() {
  ...
  const uploadActivity = async (
    ...
    try {
      ...
      let autoDetectedSport: string | null = null;
      let autoGeneratedName: string | null = null;
      ...
      if (isFileType(['.fit'])) {
        try {
          ...
          // Auto-detect sport mode from FIT data
          autoDetectedSport = parsedData.sport_mode;
          
          // Generate auto name if not provided by user
          if (!activityName && parsedData.date) {
            const namingContext: ActivityNamingContext = {
              sport_mode: parsedData.sport_mode,
              timestamp: new Date(parsedData.date),
              duration: parsedData.duration_seconds,
              distance: parsedData.distance_meters
            };
            autoGeneratedName = generateActivityName(namingContext);
          }
          ...
          activityData = {
            ...activityData,
            ...parsedData,
            // Use auto-generated name if no user name provided
            name: activityName || autoGeneratedName || parsedData.name || activityData.name,
            // Use auto-detected sport unless user explicitly chose different sport
            sport_mode: autoDetectedSport || sportMode
          };
          
          console.log('Auto-detected sport:', autoDetectedSport);
      ...
      } else if (isFileType(['.gpx'])) {
        try {
          console.log('Parsing GPX file...');
          const parsedData = await parseGPXFile(file);
          console.log('GPX file parsed successfully:', parsedData);
          
          // Auto-detect sport mode from GPX data or GPS analysis
          autoDetectedSport = parsedData.sport_mode;
          
          // Try GPS-based inference if sport not detected from file
          if (!autoDetectedSport || autoDetectedSport === 'cycling') {
            const gpsAnalysis = analyzeGPSTrack(parsedData.trackPoints);
            const inferredSport = inferActivityTypeFromGPS(gpsAnalysis);
            if (inferredSport.confidence > 0.6) {
              autoDetectedSport = inferredSport.sport_mode;
              console.log('GPS-inferred sport:', autoDetectedSport, 'confidence:', inferredSport.confidence);
            }
          }
          ...
          if (!activityName && parsedData.date) {
            const namingContext: ActivityNamingContext = {
              sport_mode: autoDetectedSport || 'cycling',
              timestamp: new Date(parsedData.date),
              duration: parsedData.duration_seconds,
              distance: parsedData.distance_meters
            };
          ...
          console.warn('Failed to parse GPX file, saving basic data:', parseError);
        }
      } else if (isFileType(['.tcx'])) {
        try {
          console.log('Parsing TCX file...');
          const parsedData = await parseTCXFile(file);
          console.log('TCX file parsed successfully:', parsedData);
          
          // Auto-detect sport mode from TCX data
          autoDetectedSport = parsedData.sport_mode;
          ...
          console.warn('Failed to parse TCX file, saving basic data:', parseError);
        }
      }
