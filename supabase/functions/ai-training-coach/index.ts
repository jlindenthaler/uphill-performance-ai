...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Incoming task:", { task, userId, sportMode });
    ...
    switch (task) {
      case "daily_recommendations":
        aiResponse = await getDailyRecommendations(trainingContext, { sport_mode: sportMode });
        break;
      case "chat_assistant":
        aiResponse = await getChatResponse(trainingContext, { sport_mode: sportMode, message: context?.message ?? "" });
        break;
      case "activity_analysis":
        aiResponse = await getActivityAnalysis(trainingContext, { activity_data: context?.activity_data ?? {}, sport_mode: sportMode });
        break;
      case "workout_generation":
        aiResponse = await generateWorkout(trainingContext, { requirements: context?.requirements ?? {}, sport_mode: sportMode });
        break;
      case "math_analysis":
        aiResponse = await runMathAnalysis(trainingContext, { math_payload: context?.math_payload ?? {} });
        break;
      case "historical_analysis":
        aiResponse = await runHistoricalAnalysis(trainingContext, { history_payload: context?.history_payload ?? {}, sport_mode: sportMode });
        break;
      default:
...
// ====================
function calculateFTP(lab: any, cp: any, mmp90: any[]) {
  ...
  if (labFresh && lab?.lt2_power > 0)       { ftp = Math.round(lab.lt2_power); src = "lab_lt2_fresh"; }
  else if (labFresh && lab?.vt2_power > 0)  { ftp = Math.round(lab.vt2_power); src = "lab_vt2_fresh"; }

  // 2) Fresh CP
  else if (cpFresh && cp?.cp_watts > 0)     { ftp = Math.round(cp.cp_watts);   src = "cp_fresh"; }
  ...
  if (!ftp && mmp90?.length) {
    ...
    if (oneHour > 0)      { ftp = Math.round(oneHour);       src = "mmp_90d_1h"; }
    else if (twenty > 0)  { ftp = Math.round(twenty * 0.95); src = "mmp_90d_20m_est"; }
    else if (five > 0)    { ftp = Math.round(five * 0.90);   src = "mmp_90d_5m_est"; }
  }
  ...
  if (!ftp) {
    ...
    if (staleLab && staleCP) {
      if (labAge <= cpAge) { ftp = Math.round(staleLab); src = "lab_threshold_stale"; }
      else                 { ftp = Math.round(staleCP);  src = "cp_stale"; }
    } else if (staleLab)   { ftp = Math.round(staleLab); src = "lab_threshold_stale"; }
      else if (staleCP)    { ftp = Math.round(staleCP);  src = "cp_stale"; }
  }

  console.log("FTP computed:", { ftp, src, labAgeDays: labAge, cpAgeDays: cpAge, labFresh, cpFresh });
  return { ftp, ftpSource: src, recency: { labAgeDays: labAge, cpAgeDays: cpAge, labFresh, cpFresh } };
}

// ====================
// 🧩 Small helper to surface thresholds concisely
// ====================
function compactPhysioSummary(ctx: any) {
...
// ====================
async function getTrainingContext(supabase: any, userId: string, sportMode: string) {
  ...
  const { data: trainingHistory, error: thErr } = await supabase
  ...
  const { data: mmp90 } = await supabase
    .from("power_profile")
  ...
  const { data: phys } = await supabase
    ...
    .select("metabolic_flexibility, hrv_rmssd, sleep_hours, sleep_quality, stress_level, fat_max_rate, fat_max_intensity, carb_max_rate, hydration_target, notes, nutrition_strategy, created_at")
    .eq("user_id", userId)
    .eq("sport_mode", sportMode)
  ...
  const { data: weeklyProgressRows } = await supabase
    .from("training_history")
    .select("tss")
    .eq("user_id", userId)
    .eq("sport", sportMode)
  ...
  const currentTLI =
    weeklyProgressRows?.reduce((s: number, r: any) => s + (r.tss || 0), 0) || 0;
  const currentSessions =
    recentActivities?.filter((a: any) => new Date(a.date) >= weekStart).length || 0;

  const { ftp, ftpSource, recency } = calculateFTP(lab, cp, mmp90 || []);

  console.log("Context summary:", {
    userId, sportMode, since: since42,
    th_count: trainingHistory?.length || 0,
    ra_count: recentActivities?.length || 0,
    goals: goals?.length || 0,
    tsb: currentTSB,
    ftp, ftpSource,
    labFresh: recency.labFresh, cpFresh: recency.cpFresh,
  });
  ...
  return {
    ...
    staleness: recency, // { labAgeDays, cpAgeDays, labFresh, cpFresh }
  };
}
...
async function getDailyRecommendations(ctx: any, _reqCtx: any) {
  const systemPrompt = `
...
- If prescribing structure, keep it brief (only what’s necessary).
- ≤ 140 words. No essays. No questions back.
`.trim();

  const nextGoal = ctx.active_goals?.[0]
    ? `${ctx.active_goals[0].name} on ${ctx.active_goals[0].event_date}`
    : "No upcoming goal";

  const physioLine = compactPhysioSummary(ctx);
  const freshness = ctx.staleness
    ? `Data Freshness: Lab ${ctx.staleness.labFresh ? "fresh" : `stale (${ctx.staleness.labAgeDays}d)`}, CP ${ctx.staleness.cpFresh ? "fresh" : `stale (${ctx.staleness.cpAgeDays}d)`}`
    : "";
...
async function getChatResponse(ctx: any, reqCtx: any) {
  const systemPrompt = `
...
async function getActivityAnalysis(ctx: any, reqCtx: any) {
  const systemPrompt = `
...
async function generateWorkout(_ctx: any, reqCtx: any) {
  const systemPrompt = `
...
// Math — DeepSeek
async function runMathAnalysis(_ctx: any, reqCtx: any) {
  const systemPrompt = `
...
async function runHistoricalAnalysis(ctx: any, reqCtx: any) {
  const systemPrompt = `
...
// Helpers
// ====================
const TSB_BANDS = { VERY_TIRED: -25, AT_RISK: -10, OPTIMAL: 5, RESTED: 10, RACE_READY: 25 };
...
function getFallbackResponse(isDaily: boolean) {
  if (isDaily) {
