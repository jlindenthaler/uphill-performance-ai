      ...
      const aiResult = await fetch(LLM_URL, {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: 'You are a world-class endurance training coach and plan architect. Analyze the athlete data deeply and generate a progressive, personalized training plan. Output ONLY valid JSON, no markdown, no explanations.'
            },
            {
              role: 'user',
              content: prompt
...
function generateBlockStructure(totalWeeks, style) {
  if (totalWeeks < 8) {
    return [
      {
        name: 'Build',
        weeks: totalWeeks - 2,
        intent: 'Build fitness and intensity tolerance'
      },
      {
        name: 'Taper',
        weeks: 2,
        intent: 'Freshen and sharpen for event'
      }
    ];
    ...
    return [
      {
        ...
        intent: 'Aerobic foundation and endurance'
      },
      {
        name: 'Build',
        weeks: Math.floor(totalWeeks * 0.5),
        intent: 'Intensity and race-specific work'
      },
      {
        name: 'Taper',
        weeks: Math.ceil(totalWeeks * 0.2),
        intent: 'Recovery and sharpening'
      }
    ];
  } else {
    return [
      ...
      },
      {
        ...
        intent: 'Threshold and tempo work'
      },
      {
        name: 'Build 2',
        weeks: Math.floor(totalWeeks * 0.25),
        intent: 'VO2max and race specificity'
      },
      {
        name: 'Taper',
        weeks: Math.ceil(totalWeeks * 0.15),
        intent: 'Freshen and maintain sharpness'
      }
    ];
  }
