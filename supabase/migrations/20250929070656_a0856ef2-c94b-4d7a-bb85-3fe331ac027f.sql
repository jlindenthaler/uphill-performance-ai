...
-- Drop and recreate the Strava token functions with proper pgcrypto usage
...
-- Create function to store Strava tokens securely
...
BEGIN
  ...
  -- Store encrypted tokens using pgcrypto
  INSERT INTO public.encrypted_strava_tokens (
    ...
    p_user_id,
    encode(encrypt(p_access_token::bytea, encryption_key::bytea, 'aes'), 'base64'),
    encode(encrypt(p_refresh_token::bytea, encryption_key::bytea, 'aes'), 'base64'),
    p_expires_at,
    p_scope,
    p_athlete_id
  )
  ON CONFLICT (user_id) DO UPDATE SET
    access_token_hash = encode(encrypt(p_access_token::bytea, encryption_key::bytea, 'aes'), 'base64'),
    refresh_token_hash = encode(encrypt(p_refresh_token::bytea, encryption_key::bytea, 'aes'), 'base64'),
...
-- Create function to retrieve Strava tokens securely
...
BEGIN
  ...
  IF FOUND THEN
    ...
    RETURN QUERY SELECT 
      convert_from(decrypt(decode(token_record.access_token_hash, 'base64'), encryption_key::bytea, 'aes'), 'UTF8'),
      convert_from(decrypt(decode(token_record.refresh_token_hash, 'base64'), encryption_key::bytea, 'aes'), 'UTF8'),
